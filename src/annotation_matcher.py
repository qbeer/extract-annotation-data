...
def acquire_annotations(annotation_path=os.getcwd() +
    ...
    with open(annotation_path, "r") as read_file:
        for line in read_file.readlines():
            ...
            annotation_id = splitted_on_double_dot[0][1].strip()
            print(splitted_on_double_dot[1])
            image_id = int(splitted_on_double_dot[1][1].strip())
            polygon = splitted_on_double_dot[7][1].strip().replace(
                'POLYGON ((', '').replace('))', '').split(' ')
            karolinska_name = id_to_karolinska[image_id]
            elte_name = karolinska_to_elte[karolinska_name]
            print(elte_name, image_id, karolinska_name)